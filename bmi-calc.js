...
function getBmiClass () {
    const bmiResult = calculateBmi().toFixed(2);
    let result; 

    if (bmiResult < 18.5) {
        result = `Your BMI is ${bmiResult}, this means that you are underweight`;
    } else if (bmiResult > 18.5 && bmiResult < 25) {
        result = `Your BMI is ${bmiResult}, this means that you have a normal weight`;
    } else if (bmiResult > 25 && bmiResult < 30) {
        result = `Your BMI is ${bmiResult}, this means that you are overweight`;
